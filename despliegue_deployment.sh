...
#oc new-app -e TZ=Europe/Madrid -p APPLICATION_NAME=${APLICACION} -p VERSION_TAG=${VERSION_MAJOR} -p NAMESPACE=${NAMESPACE_DESTINO} -p DOMAIN_SUFFIX=.acme -n ${NAMESPACE_DESTINO} --template ${NAMESPACE_ACOC}/${TEMPLATE}
oc new-app ${APLICACION}:${VERSION_MAJOR} ${APLICACION}:${VERSION_MAJOR} -n ${NAMESPACE_DESTINO}
